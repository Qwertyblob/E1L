...
-- a timestamp records when ArchivalService archived the row. Archived bookings are
-- hidden from all listings (they appear deleted); archived slots stay visible to
-- admins, flagged as archived.
...
-- The public availability and booking-listing queries filter on archived_at IS NULL;
-- partial indexes keep the active subset cheap to scan as archived rows accumulate.
